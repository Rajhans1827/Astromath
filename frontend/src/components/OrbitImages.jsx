...
// Optimized for AstroMath Navagraha Cosmic Visualizer
...
export default function OrbitImages({
  ...
  className = '',
  showPath = true,
  pathColor = 'rgba(245, 158, 11, 0.22)',
  pathWidth = 1.5,
  easing = 'linear',
  paused = false,
  centerContent,
  ...
  const items = images.map((src, index) => (
    <img
      key={typeof src === 'string' ? src : index}
      src={typeof src === 'string' ? src : src.src}
      alt={src.name || `${altPrefix} ${index + 1}`}
      title={src.name || `Planet ${index + 1}`}
      draggable={false}
