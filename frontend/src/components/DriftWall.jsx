...
const DriftWall = ({
  ...
  const effectiveColumns = useMemo(() => {
    if (containerWidth < 640) return Math.min(columns, 3);
    if (containerWidth < 960) return Math.min(columns, 4);
    return columns;
  }, [containerWidth, columns]);

  // Responsive tile sizing
  const effectiveTileWidth = useMemo(() => {
    if (containerWidth < 640) return 240;
    return tileWidth;
  }, [containerWidth, tileWidth]);

  const effectiveTileHeight = useMemo(() => {
    if (containerWidth < 640) return 158;
    return tileHeight;
  }, [containerWidth, tileHeight]);
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Drifting wall of verified user experiences"
    >
